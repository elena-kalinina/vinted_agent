...
url = "https://www.vinted.fr/chaussures?search_text=&brand_id[]=&color_id[]="
url_search_txt = "https://www.vinted.fr/chaussures?search_text="
...
def searchVinted(print_url=True,searchText="",catalog=[],color=[],brand=[],size=[],material=[],status=[],country=[],price_to=1000000,price_from=0,currency="EUR",per_page=120,page=1):
    ...
    url_search = f"https://www.vinted.fr/api/v2/catalog/items?search_text={searchText}"
            # ("https://www.vinted.fr/vetements?search_text="+searchText)
    url_params = {
        "per_page":per_page,
        "page":page,
    ...
    req = requests.get(url_search,params=url_params, headers=HEADERS, impersonate="chrome120")
    if print_url:
        print(req.url)
    if req.status_code == 200:
        ...
        return items  # {"items":items["byId"],"searchParams":items["catalogItems"]}
    else:
